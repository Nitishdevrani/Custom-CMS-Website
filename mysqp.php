...
$msg = "";
  ...
  if(array_key_exists("id",$_SESSION)) {
	...
	if(!$_POST["cost"]) {
          $error .= "Please enter Price of this Paper.<br>";
        }
        ...
        $aim = "solved_Papers/".basename($upld);
        ...
        if($error != "") {
          ...
          $query = "INSERT INTO `solvedPapers`(`solved_userId`,`solved_uniName`,`solved_uniCourse`, `solved_uniSem`, `solved_pprSub`, `solved_ppr`,`ppr_price`)
                    VALUES(
		      ...
		      '".mysqli_real_escape_string($link,$_POST['qsSub'])."',
                      '$upld',
		      '".mysqli_real_escape_string($link,$_POST['cost'])."'
                    )";
...
<?php include("nav.php");?>
 ...
 <div class="container-fluid bg-light"  style="padding:100px;">
       ...
       <form class="boxchota" style="border-radius:30px;" method="post" enctype="multipart/form-data">
         <h1><center>ADD MORE SOLVED QUESTION PAPERS!!</center></h1>
...
<?php
                         ...
                         $query = "SELECT `solved_uniName`,`solved_uniCourse`, `solved_uniSem`,`solved_pprSub`,`ppr_price` FROM `solvedPapers` WHERE solved_userId = '$_SESSION[id]'";
...
//                                                                  ' .urlencode($row['Title']). '
                       ...
                       while($row = mysqli_fetch_assoc($result))
                       {
                           echo "
                                <tr class='bg-light'>
                                <td scope='col'>".$row['solved_uniName']."</td>
				<td scope='col'>".$row['solved_uniCourse']."</td>
				<td scope='col'>".$row['solved_uniSem']."</td>
				<td scope='col'>".$row['solved_pprSub']."</td>
				<td scope='col'>".$row['ppr_price']."</td>
                                </tr>";
                       }
